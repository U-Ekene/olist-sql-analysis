...
-- Finding: 99,441 — matches total orders, since each order gets 
-- its own customer_id.
...
-- customer_unique_id is the real person-level identifier.
-- Finding: 96,096 unique people vs 99,441 orders, so only ~3.4% 
-- of orders came from repeat buyers. Low repeat rate for e-commerce.
SELECT COUNT(DISTINCT customer_unique_id) AS unique_people
FROM customers;

-- Q7: Top 10 cities by order count in São Paulo state.
-- Shows how concentrated orders are within a single state.
-- Finding: São Paulo city dominates with 15,540 orders — ~10× 
-- larger than #2 (Campinas, 1,444). Extreme concentration in 
-- the capital.
...
-- national level?
-- Finding: 3 states (SP, RJ, MG) account for ~67% of all orders.
-- SP alone is 42% of the national total.
